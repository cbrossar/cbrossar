    ...
    with open(backup_file, 'r') as f:
        for line_num, line in enumerate(f, 1):
            ...
            if f"-- Name: {table_name}; Type: TABLE;" in line_stripped:
                logger.info(f"Found table structure at line {line_num}: {line_stripped}")
            ...
            if line_stripped.startswith("-- Name: ") and f"; Type: TABLE;" in line_stripped and table_name not in line_stripped:
                if in_target_table:
            ...
            if line_stripped.startswith("-- Data for Name: ") and f"; Type: TABLE DATA;" in line_stripped and table_name not in line_stripped:
                if in_target_table:
            ...
            if in_target_table:
                if table_section == 'structure':
                    ...
                    if (line_stripped.startswith("DROP TABLE") or
                        line_stripped.startswith("CREATE TABLE") or 
                        line_stripped.startswith("ALTER TABLE") or
                        line_stripped.startswith("ADD CONSTRAINT") or
                        line_stripped.startswith("OWNER TO") or
                        line_stripped == "" or
                    ...
                    if (line_stripped.startswith("COPY ") or
                        line_stripped.startswith("\\.") or
                        line_stripped == "" or
                        line_stripped.startswith("--") or
                        (not line_stripped.startswith("--") and line_stripped)):
                        extracted_lines.append(line.rstrip())
