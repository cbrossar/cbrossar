...
export default async function Page() {
    ...
    const goalkeepers = optimalTeam.team.filter(player => player.element_type === 1);
    const defenders = optimalTeam.team.filter(player => player.element_type === 2);
    const midfielders = optimalTeam.team.filter(player => player.element_type === 3);
    const forwards = optimalTeam.team.filter(player => player.element_type === 4);
                        ...
                        {goalkeepers.map(player => (
                            <div key={player.id} className={styles.player}>
                                <div className={styles.playerInfo}>
                                    ...
                                    <span className={styles.points}>Points: {player.total_points}</span>
                                    <span className={styles.cost}>Cost: £{(player.now_cost / 10).toFixed(1)}m</span>
                                </div>
                            </div>
                        ))}
                    </div>
